...
% neural network.
% 
% files=dir('C:\Users\Admin\Documents\Education\MAIA-Spain\DeepLearning\2 Macular_patches_of_size_224x224\2*.tif');
% for i=1:length(files)
%    image = imread(strcat(files(i).folder, '/', files(i).name));
%    image = imresize(image, [227 227]);
%    imwrite(image, strcat('C:\Users\Admin\Documents\Education\MAIA-Spain\DeepLearning\2 Macular_patches_of_size_224x224(resized)\2\',files(i).name));
% end


images = imageDatastore('C:\Users\Admin\Documents\Education\MAIA-Spain\DeepLearning\2 Macular_patches_of_size_224x224(resized)',...
    'IncludeSubfolders',true,...
    'LabelSource','foldernames',
    );
% images.Files = images.Files{1:10000};
% images.Labels = images.Labels(1:10000, :);
...
% [images,~] = splitEachLabel(images,0.2,'randomized');
